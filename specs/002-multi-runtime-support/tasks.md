...
<!-- Status: RECONCILED -->
<!-- Prior: V1 expanded, V2 packaging revised, V3 rewritten, V4 centralized inference -->
<!-- Fresh review (2026-03-07): 9 findings → V3.12 add in_progress for spawnAgents, V4.1-V4.4 use direct regfile read for reservedPaths, V3.1 add tool_result mapping, V3.3+V3.13 nonceHash schema in lib.ts -->
---
...
- [x] **V3.12** Update ALL `buildWorkerPrompt()` callsites — pass runtime + assignmentMode. **CRITICAL (Codex fresh-R1 finding #1):** The remainingTasks loop in work.ts:146-165 must set `store.updateTask(status: "in_progress")` BEFORE building the prompt (matching the lobby path at line 127). Without this, task.done fails because it requires in_progress status:
...
- [x] **V3.13** Add worker nonce auth (Codex fresh-R1 finding #3 + R2 finding #3):
  - Update `AgentRegistration` in `lib.ts` to add optional `nonceHash?: string` field
  - Spawner generates `PI_CREW_NONCE` (randomUUID), stores SHA-256 hash via `registerSpawnedWorker(... nonceHash)`
  - `registerSpawnedWorker()` accepts `nonceHash` parameter, writes to registration file
  - CLI `validateNonce()` checks `PI_CREW_NONCE` env against stored hash for mutating commands (task.start, task.done, send, reserve)
  - Read-only commands (status, list, feed) skip validation
...
- [x] **V3.16** End-to-end manual test — configure `runtime: { worker: "claude" }`, run work action, verify Claude Code spawns and completes task via CLI
...
- [x] **V4.1** Create `crew/completion-inference.ts` — centralized completion inference shared by BOTH lobby.ts close handler AND work.ts result processing (Codex fresh-R1 finding #2, R2 finding #1, R3 finding #1):
  - `inferTaskCompletion(ctx: InferenceContext): boolean`
  - `InferenceContext` includes `reservedPaths?: string[]` for concurrency-safe attribution
  - Scopes file attribution to reserved paths when available; falls back to repo-wide with logged warning
  - Requires at least one file change to infer (exit 0 + no changes = NOT inferred)
  - Both callsites read registration file DIRECTLY (not getActiveAgents which filters dead PIDs)
...
- [x] **V4.5** Add stuck detection timer — track lastOutputTimestamp per worker, fire warning after configurable timeout. Log to feed with type "stuck" (matches FeedEventType). Don't auto-kill.
...
- [x] **P3** Close bead pi-messenger-2 (already closed with prior PR)
