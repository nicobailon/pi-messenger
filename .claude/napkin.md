...
| 2026-04-05 | Rate-limit incident deep-dive | Spawn/send waits treated terminal provider failures (429 rate_limit_error, auth/quota) like generic stalls, so sessions sat "alive" for minutes while retries churned. | Parse collaborator log events during blocking poll. If terminal provider error appears, fail immediately, surface status/type/request_id, and stop collaborator deterministically. |
...
- **Fail fast on terminal provider errors** — blocking poll loops must inspect runtime error events, not just heartbeat/log liveness. 429/401/403/quota/auth failures are terminal for the exchange and should short-circuit immediately.
