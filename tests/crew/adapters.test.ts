...
import { CodexAdapter } from "../../crew/utils/adapters/codex.js";
...
describe("ClaudeAdapter.parseProgressEvent", () => {
  ...
  it("maps user events to tool_result", () => {
...
describe("CodexAdapter.buildArgs", () => {
  ...
  it("builds minimal args for a simple prompt", () => {
    ...
    const config: AdapterConfig = { extensionDir: "/ext" };
    const args = adapter.buildArgs(task, config);
    expect(args).toEqual(["exec", "--json", "Fix the bug"]);
  });

  it("includes model with provider prefix stripped", () => {
    const task: SpawnTask = { prompt: "hello" };
    const config: AdapterConfig = { model: "openai/o4-mini", extensionDir: "" };
    const args = adapter.buildArgs(task, config);
    expect(args).toContain("-m");
    expect(args).toContain("o4-mini");
    expect(args).not.toContain("openai/o4-mini");
  });

  it("prepends system prompt to task prompt", () => {
    const task: SpawnTask = { prompt: "Do the task", systemPrompt: "Be careful" };
    const config: AdapterConfig = { extensionDir: "" };
    const args = adapter.buildArgs(task, config);
    // Last arg is the prompt with system prepended
...
describe("CodexAdapter.parseProgressEvent", () => {
  const adapter = new CodexAdapter();

  it("parses command_execution started as tool_call", () => {
    const line = JSON.stringify({
      type: "item.started",
    ...
    expect(event).toEqual({
      type: "tool_call",
      toolName: "shell",
      args: { command: "/bin/zsh -lc 'ls'" },
    });
  });

  it("parses command_execution completed as tool_result", () => {
    const line = JSON.stringify({
      type: "item.completed",
  ...
  it("parses agent_message as message", () => {
    const line = JSON.stringify({
      type: "item.completed",
      item: { id: "item_2", type: "agent_message", text: "I fixed the bug" },
    });
    const event = adapter.parseProgressEvent(line);
    expect(event).toEqual({ type: "message", content: "I fixed the bug" });
  });

  it("parses error item as error", () => {
    const line = JSON.stringify({
      type: "item.completed",
      item: { id: "item_0", type: "error", message: "Model not found" },
    });
    const event = adapter.parseProgressEvent(line);
    expect(event).toEqual({ type: "error", errorMessage: "Model not found" });
  });
  ...
  it("parses fatal error", () => {
    ...
    const event = adapter.parseProgressEvent(line);
    expect(event).toEqual({ type: "error", errorMessage: "Connection refused" });
  });

  it("parses turn.completed with usage tokens", () => {
    const line = JSON.stringify({
      type: "turn.completed",
...
describe("CodexAdapter.supportsFeature", () => {
  const adapter = new CodexAdapter();

  it("supports streaming only", () => {
    expect(adapter.supportsFeature("streaming")).toBe(true);
  });

  it("does not support system-prompt-inline, thinking, tool-restriction, extension-loading", () => {
    expect(adapter.supportsFeature("system-prompt-inline")).toBe(false);
    expect(adapter.supportsFeature("thinking")).toBe(false);
    expect(adapter.supportsFeature("tool-restriction")).toBe(false);
    expect(adapter.supportsFeature("extension-loading")).toBe(false);
    expect(adapter.supportsFeature("system-prompt-file")).toBe(false);
