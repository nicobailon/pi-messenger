...
describe("renderSessionDetailView", () => {
  it("renders session metadata in the header", () => {
    const lines = renderSessionDetailView(makeSession(), "healthy", 90, 18, Date.parse("2026-03-08T03:00:30.000Z"));
    ...
    expect(plain).toContain("2026-03-08 03:00:30Z");
  });

  it("renders distinguishable thinking, tool, progress, execution, error, and completion entries", () => {
    const lines = renderSessionDetailView(makeSession({ status: "ended" }), "healthy", 100, 18, Date.parse("2026-03-08T03:00:30.000Z"));
    const plain = stripDetailAnsi(lines.join("\n"));
...
describe("SessionDetailView", () => {
  ...
  it("supports inspectability after completion by disabling auto-follow and scrolling older entries", () => {
    ...
    const view = new SessionDetailView({ maxHeight: 12 });
    view.setSession(session, { health: "degraded" as HealthStatus });
    view.handleInput("f");
